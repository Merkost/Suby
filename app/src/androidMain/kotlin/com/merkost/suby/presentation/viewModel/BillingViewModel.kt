...
import suby.app.generated.resources.error_loading_products
import suby.app.generated.resources.error_no_purchase_to_restore
import suby.app.generated.resources.error_purchase_failed
import suby.app.generated.resources.error_restore_purchase
import suby.app.generated.resources.purchase_restored
    ...
    fun loadProducts() {
        viewModelScope.launch {
            ...
            billingService.getMainOffering().fold(
                ...
                onFailure = {
                    _uiEvent.emit(
                        UiEvent.ShowError(messageRes = Res.string.error_loading_products)
                    )
                }
            )
    ...
    fun purchase(activity: Activity) {
        viewModelScope.launch {
            ...
            billingService.purchase(activity, product, null).fold(
                ...
                onFailure = {
                    _uiEvent.emit(
                        UiEvent.ShowError(messageRes = Res.string.error_purchase_failed)
                    )
                }
            )
            _uiState.value = BaseUiState.Success(Unit)
            ...
            billingService.restorePurchase().fold(
                onSuccess = { entitlements ->
                    if (entitlements.none { it.isActive }) {
                        _uiEvent.emit(
                            UiEvent.ShowError(messageRes = Res.string.error_no_purchase_to_restore)
                        )
                        return@fold
                    }
                    appSettings.saveHasPremium(entitlements.any { it.isActive })
                    _uiEvent.emit(
                        UiEvent.ShowSuccess(messageRes = Res.string.purchase_restored)
                    )
                },
                onFailure = {
                    _uiEvent.emit(
                        UiEvent.ShowError(messageRes = Res.string.error_restore_purchase)
                    )
                },
            )
            _uiState.value = BaseUiState.Success(Unit)
